...
export default class Flogin extends Component {
  ...
  handelSubmit = e => {
    ...
    axios.post("http://localhost:5000/api/login1",JSON.stringify({"name":this.state.name,"email": this.state.email, "password": this.state.password }), axiosConfig)
      .then(function (response) {
        if (response.data.redir === 'faculty') {
      ...
      <div style={{
        ...
        <Card div style={{
          ...
          <Card.Body>
            ...
            <p>If not new user Register here  <Link to="/sign-up1">sign-up</Link> </p>
          </Card.Body>
        </Card>
      </div>
    );
